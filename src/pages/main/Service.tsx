...
const entries = serviceData as Entry[];
...
export function MyService() {
  ...
  return <>
   ...
   {older.length > 0 &&
      <Collapsible title='Earlier service' light
         count={`${olderCount} ${olderCount === 1 ? 'entry' : 'entries'}`}>
         <div className='service-timeline service-timeline-older'>{older}</div>
      </Collapsible>
   }
 </>;
}
